    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['share_post_comment'])) {
        if (is_uploaded_file($_FILES['post_comment_image']['tmp_name'])) {
            ...
            if (move_uploaded_file($source_path, $target_path)) {
                if ($file_extension == 'jpg' || $file_extension == 'png' || $file_extension == 'jpeg') {
                    ...
                    $post_comment_image_err = "Please enter a valid file extension.";
                }
            } else {
                $post_comment_image_err = "We encountered a problem while saving your image.";
            }
        }

        if (empty(trim($_POST["post_comment_title"]))) {
            $post_comment_title_err = "Please fill out the comment field.";
        } elseif (strlen(trim($_POST["post_comment_title"])) > 280) {
            $post_comment_title_err = "Your comment must be a maximum of 280 characters.";
        } else {
        ...
        if (empty($post_comment_title_err) && empty($post_comment_image_err)) {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    $$share_comment_success = "Your post has been successfully shared.";
                } else {
                    $share_post_error = "Sorry, your post could not be shared at this time.";
                }
            } else {
                $share_post_error = "Sorry, your post could not be shared right now.";
            }
        }
    }
...
if (empty($row) || empty($post_id)) { ?>
    <div class="pt-16 text-sm text-center">Sorry, the post you are looking for could not be found.</div>
    <?php } else {
    include($_SERVER['DOCUMENT_ROOT'] . "/social2/social/components/content/components/post-card.php");
    ...
    if ($commentData->num_rows > 0) {
        ...
        <div class="py-16 text-sm text-center">There are no comments here. Why Don't you start...</div>
<?php }
